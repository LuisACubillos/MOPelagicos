...
GLOBALS_SECTION
 ...
 ofstream rep10("10Fmort_hist.txt",ios::app);
 ofstream rep11("09Desovante_hist.txt",ios::app);
 ofstream rep12("08BiomTotal_hist.txt",ios::app);
 ofstream rep13("10Reclutas_hist.txt",ios::app);   
 ofstream rep14("06RPR_est.txt",ios::app);   
 ofstream rep15("06RPR_op.txt",ios::app);  
 ofstream rep16("11RPR_hist.txt",ios::app);
 ofstream rep17("07RPRdin_op.txt",ios::app);   
 ofstream rep18("07RPRdin_est.txt",ios::app);  
 ofstream rep19("12RPRdin_hist.txt",ios::app);   
...
// 12. Puntos biológicos de referencia
 ...
 // 13. PROYECCION, CRITERIOS DE EXPLOTACIÓN Y ESCENARIOS DE RECLUTAMIENTO
  ...
  init_int SrType 
  init_int ErrorObs 
  init_matrix States(1,2,1,2)
  init_matrix mu_std(1,2,1,2)
  init_vector fut_states(nanos+1,nanos+nproy)
...
// Proyeccion futura
	 ...
	 //Datos simulados
	  ...
	  matrix keep_RBdin(1,npbr,nanos+1,nanos+nproy);
	  matrix des_RBdin(1,npbr,nanos+1,nanos+nproy);
...
FUNCTION Eval_Mop
		...
		simname="MAE0920c.dat";//datos del estimador
	   	dvector CatchNow(1,npbr);//capturas del estimador
	   	dvector eval_now(1,6);//Indicadores de estatus del estimador:BT,BD,R,F,B/Bo
	 	...
	 	for(int l=1;l<=npbr;l++){
			...
			Wm_fut=Wmed(nanos);//asume el peso medio del año mas reciente
	 		...
	 		for(i=nanos+1;i<=nanos+nproy;i++)
	 		{
	 			 ...
	 			 //Modelo operativo con sigma_r definido en linea 524 
				...
				if(fut_states(i)==1){
					...
					N_fut(1)=mfexp(mu_std(1,1)+rec_epsilon_fut(i) + 0.5*square(sigr));
				}
				if(fut_states(i)==2){
					sigr = mu_std(2,2);
					rec_epsilon_fut(i)=epsilon_rec_vect(i)*sigr;
					N_fut(1)=mfexp(mu_std(2,1)+rec_epsilon_fut(i) + 0.5*square(sigr));
				}
			    ...
			    Nv_fut    = N_fut;// solo para empezar los calculos
			    Nv_fut(2,nedades)=++Nv_fut(1,nedades-1)*exp(-1.0*M);
			    Nv_fut(nedades)+=Nv_fut(nedades)*exp(-1.0*M);		
		        //Desovante
		        NMD_fut = elem_prod(elem_prod(N_fut,mfexp(-dt(3)*(Zpbr))),msex);	//numero desovante
				NMDo_fut = elem_prod(N_fut*mfexp(-dt(3)*M),msex);	//numero desovante no explotado
		        BMmph_fut(i)=sum(elem_prod(Win_fut,NMD_fut));
					
		        BDp=sum(elem_prod(NMD_fut,Win_fut));	
		        BTp=sum(elem_prod(N_fut,Win_fut));
				//Bo DINAMICO
				BDodin =	sum(elem_prod(NMDo_fut,Win_fut));   	
