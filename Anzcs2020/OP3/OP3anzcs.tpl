...
// Proyeccion futura
	 ...
	 //Datos simulados
	  ...
	  vector rec_epsilon(nanos,nanos+nproy);
	    ...
	    //cout << "ctp"<< CatchNow << endl;exit(1);
	 	...
	 	for(int l=1;l<=npbr;l++){
			...
			Wm_fut=Wmed(nanos);//asume el peso medio del año mas reciente
	 	    ...
	 	    sel=Sel_f(nanos);
			rec_epsilon(nanos)=log_desv_Rt(nanos)+0.5*square(sigr); //copia la desv del ultimo año

	 		for(i=nanos+1;i<=nanos+nproy;i++)
	 		{
	 			rec_epsilon(i) = rho_r*rec_epsilon(i-1)+epsilon_rec_vect(i)*sigr*pow(1-rho_r,0.5);
				rec_epsilon_fut(i)=rec_epsilon(i); //Modelo operativo con sigma_r definido en linea 524 
